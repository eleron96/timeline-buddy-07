...
export const WidgetEditorDialog: React.FC<WidgetEditorDialogProps> = ({
  ...
  useEffect(() => {
    ...
    if (initialWidget) {
      ...
      setSize(initialWidget.size ?? (initialWidget.type === 'kpi' ? 'small' : 'medium'));
  ...
  useEffect(() => {
    ...
    setSize(type === 'kpi' ? 'small' : 'medium');
  }, [initialWidget, type]);

  const showGroupBy = type !== 'kpi';
  ...
  const handleSave = () => {
    ...
    const normalizedGroupBy = normalizedType === 'kpi' ? 'none' : groupBy;
    ...
    const nextWidget: DashboardWidget = {
      ...
      period,
      size,
      barPalette: normalizedType === 'bar' ? barPalette : undefined,
      statusFilter,
      statusIds: showCustomStatuses ? statusIds : [],
