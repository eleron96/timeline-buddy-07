...
ssh "$host" "if docker ps --format '{{.Names}}' | grep -qx 'motio-caddy'; then docker exec motio-caddy caddy reload --config /etc/caddy/Caddyfile >/dev/null 2>&1 || docker restart motio-caddy >/dev/null; fi"
