...
const composePath = path.join(process.cwd(), '.env.compose');
  const defaultValues = {
...
const ensureCompose = () => {
  ...
  fs.writeFileSync(composePath, env);
  return true;
};

const created = ensureCompose();
if (created) {
  console.log('Generated .env.compose');
}

const env = fs.readFileSync(composePath, 'utf8');
...
if (updated) {
  ...
  fs.writeFileSync(composePath, serialized);
  console.log('Updated .env.compose with defaults');
}
...
if (!anonKey) {
  console.error('ANON_KEY missing in .env.compose');
  process.exit(1);
}

const viteEnv = `VITE_SUPABASE_URL=${apiUrl}\nVITE_SUPABASE_ANON_KEY=${anonKey}\n`;
fs.writeFileSync(path.join(process.cwd(), '.env.local'), viteEnv);
...
get_env_value() {
  local key="$1"
  local line
  line=$(grep -E "^${key}=" .env.compose | head -n1 || true)
  echo "${line#*=}"
}
...
docker compose --env-file .env.compose up -d auth rest functions gateway

docker compose --env-file .env.compose run --rm migrate

docker compose --env-file .env.compose up web
