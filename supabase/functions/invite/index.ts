...
const ensureAdmin = async (userId: string, email?: string | null) => {
  if (reserveAdminEmail && email && email.toLowerCase() === reserveAdminEmail) {
    return true;
  }
...
const handleAdminUsers = async (req: Request) => {
  ...
  const isAdmin = await ensureAdmin(authResult.user.id, authResult.user.email);
  if (!isAdmin) {
    return jsonResponse({ error: "Forbidden" }, 403);
  }
